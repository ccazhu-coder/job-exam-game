...
(function(){
  ...
  async function api(obj,targetId){
    ...
    try{
      ...
      return res;
    }catch(e){res={success:false,message:'系統暫時無法完成操作，請稍後再試。'};if(target)target.textContent=res.message;return res}
  }
  function safeDisplay(v){const text=String(v==null?'':v);if(!text)return'';if(/[A-Za-z]{2,}|https?:|script\.google|exception|stack|undefined|JSON|API|Backend|Debug/i.test(text))return'已隱藏系統細節';return text}
  function labelKey(k){return({Email:'電子信箱',folderId:'資料夾連結',driveUrl:'檔案連結'}[k]||String(k).replace(/ID/g,'編號'))}
  function cellValue(k,v){if(/Email|mail|Url|URL|連結|folderId|driveUrl/i.test(k))return v?'已隱藏':'';return safeDisplay(v)}
  function format(res){if(!res)return'系統沒有回應';let text=safeDisplay(res.message||'操作完成');const d=res.data||{};if(d.新增筆數!==undefined)text+='\n新增筆數：'+d.新增筆數;return text}
  function dateTW(id){const v=$(id)&&$(id).value;if(!v)return'';const a=v.split('-');return a[0]+'/'+a[1]+'/'+a[2]}
  function renderTable(res,id){const box=$(id);if(!box)return;const d=(res&&res.data)||{};const arr=d.rows||d.資料||d.廠商||d.學員||d.文件||d.歸檔清單||d.任務||d.變更紀錄||d.版本紀錄||d.未收款||[];if(!Array.isArray(arr)||!arr.length){box.innerHTML='<div class="empty">查無資料</div>';return}const keys=Object.keys(arr[0]).filter(k=>k!=='_row').slice(0,10);box.innerHTML='<table><thead><tr>'+keys.map(k=>'<th>'+esc(labelKey(k))+'</th>').join('')+'</tr></thead><tbody>'+arr.slice(0,80).map(r=>'<tr>'+keys.map(k=>'<td>'+esc(cellValue(k,r[k]??''))+'</td>').join('')+'</tr>').join('')+'</tbody></table>'}
  ...
  function nav(){return '<a href="./dashboard.html">營運首頁</a><a href="./projects.html">專案</a><a href="./activities.html">活動</a><a href="./vendors.html">廠商</a><a href="./crm.html">客戶關係</a><a href="./archive.html">核銷</a><a href="./finance-secretary.html">財務</a>'}
