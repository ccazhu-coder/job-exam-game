/* GovOps OS Frontend SaaS Runtime v1.0.1 */
(function(){
  ...
  function safeJSON(key,fallback){try{const raw=localStorage.getItem(key);if(!raw)return fallback||{};const parsed=JSON.parse(raw);return parsed&&typeof parsed==='object'?parsed:(fallback||{})}catch(e){return fallback||{}}}
  ...
  function getAuth(){const a=safeJSON('govops_auth',{});a.plan=normalizePlan(a.plan||getProfile().plan);a.userRole=normalizeRole(a.userRole||a.role||getProfile().role);return a}
  function getUsage(){return Object.assign({events:0,crm:0,users:1,ai:0},safeJSON('govops_usage',{}))}
  function saveUsage(u){localStorage.setItem('govops_usage',JSON.stringify(Object.assign(getUsage(),u||{})))}
  ...
  window.GovOpsSaaS={PLAN_LIMITS,ROLE_PERMISSIONS,normalizePlan,normalizeRole,getProfile,getAuth,getUsage,saveUsage,getPlan,hasFeature,toBackendParams,repairStoredSession};
  repairStoredSession();
})();
